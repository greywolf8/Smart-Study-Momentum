...
export class AIStudyPlanner {
  ...
  generateDailyPlan(subjects: string[], availableTime: number, deadlines: any[]): StudyPlan {
  ...
  generateMicroTasks(subject: string, sessionDuration: number, difficulty: 'easy' | 'medium' | 'hard'): MicroTask[] {
  ...
  createFocusSession(subject: string, duration: number): FocusSession {
    const difficulty = this.getDifficultyForSubject(subject, 0);
    const microTasks = this.generateMicroTasks(subject, duration, difficulty);
